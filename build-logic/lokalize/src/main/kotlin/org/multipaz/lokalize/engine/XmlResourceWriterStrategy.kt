...
/**
 * Android XML resource writer strategy.
 * Generates standard Android strings.xml files with support for strings, plurals, and string-arrays.
 */
class XmlResourceWriterStrategy : ResourceWriterStrategy {
    ...
    override fun mergeBundle(
        ...
        val entriesToAdd = createBundleEntries(bundle, existingKeys)
    ...
    override fun writeBundle(file: File, bundle: LocaleBundle) {
        ...
        // Add all strings
        bundle.strings.values.forEach { entry ->
            createStringElement(document, entry).appendTo(root)
        }

        // Add all plurals
        bundle.plurals.values.forEach { entry ->
            createPluralElement(document, entry).appendTo(root)
        }

        // Add all arrays
        bundle.arrays.values.forEach { entry ->
     ...
     */
    private inner class BundleEntries(
    ...
    private fun createBundleEntries(bundle: LocaleBundle, existing: ExistingKeys): BundleEntries {
